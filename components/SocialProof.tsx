    ...
    <section className="py-10 md:py-14 bg-zinc-50/50">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {cases.map((c, i) => (
            <div key={i} className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm hover:shadow-xl transition-all group">
              <div className="flex items-center gap-4 mb-5">
                <div className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center text-white font-bold text-xs">{c.name[0]}</div>
                <div>
                ...
                <div className="ml-auto px-3 py-1 bg-amber-50 text-amber-600 text-[8px] font-black uppercase rounded-full border border-amber-100 italic">
                  {c.verdict}
                </div>
              </div>
              <p className="text-zinc-600 italic font-medium text-sm leading-relaxed">"{c.text}"</p>
            </div>
          ))}
        </div>

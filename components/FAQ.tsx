...
const FAQ: React.FC = () => {
  ...
  return (
    <section className="max-w-4xl mx-auto py-12 md:py-16 px-6 border-t border-zinc-100">
      <h3 className="text-[10px] font-black text-amber-500 uppercase tracking-[0.4em] mb-6 text-center">Frequently Asked Questions</h3>
      <div className="space-y-3">
        {faqs.map((faq, i) => (
          <div key={i} className="border border-zinc-100 rounded-xl overflow-hidden bg-white hover:border-amber-100 transition-colors">
            <button 
              onClick={() => setOpenIndex(openIndex === i ? null : i)}
              className="w-full px-6 py-5 text-left flex justify-between items-center group"
            >
              <span className="text-xs font-bold text-zinc-800 tracking-tight group-hover:text-zinc-900 transition-colors">{faq.q}</span>

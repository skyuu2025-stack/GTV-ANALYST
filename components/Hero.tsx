...
const Hero: React.FC<HeroProps> = ({ onStart }) => {
  return (
    <div className="bg-white min-h-[calc(100vh-80px)] flex flex-col items-center justify-center px-4 md:px-6 py-10 md:py-20 text-center animate-fade-in">
      <div className="inline-block px-5 md:px-8 py-2 md:py-3 bg-[#FFFBEB] border border-[#FEF3C7] rounded-full text-[#D97706] text-[9px] md:text-[13px] font-black uppercase tracking-[0.2em] mb-8 md:12 shadow-sm italic">
        The Industry Standard AI Counsel
      </div>
      ...
      <h1 className="text-[32px] md:text-[84px] font-bold text-[#1A1A1A] max-w-5xl leading-[1.15] md:leading-[1.1] mb-8 md:mb-10 tracking-tighter">
        Will You Qualify for <br/>
        <span className="font-serif italic text-[#D4AF37] font-medium decoration-1 underline-offset-8">Global Talent?</span>
      </h1>
      
      <p className="text-zinc-400 text-sm md:text-[22px] max-w-2xl mx-auto leading-relaxed mb-10 md:mb-16 font-medium opacity-80 px-4 italic">
        Get an instant, professional success roadmap using the latest Home Office criteria. Trusted by creators in Fashion, Tech, and Visual Arts.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-16 w-full max-w-4xl px-2">
        {[
          ...
          { icon: 'fa-file-invoice-dollar', label: 'Consultant-Grade' }
        ].map((f, i) => (
